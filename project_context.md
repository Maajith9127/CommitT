...
4.  **Offline SQLite Synchronization Engine**: `commit.db` seamlessly mirrors dynamic Convex states locally, providing absolute resilience during offline use.
    *   **Triple-Write Architecture (Saga Pattern)**: Operations flow securely from Convex (Cloud) → SQLite (Local) → Kotlin (Hardware Alarms).
        ...
        *   **Reconciliation Safety Net**: If the internet dies *during* the rollback, leaving the cloud with a "Ghost Commitment", the `HydrationEngine` acts as the ultimate garbage collector. The next time the app opens, the engine discovers the ghost task and cleanly syncs it down to the Local DB, self-healing the state drift.
