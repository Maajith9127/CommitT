...
_A tutorial on creating a native view that renders a WebView with Expo Modules API._
...
After that, you'll see the [Expo Modules API overview page](https://docs.expo.dev/modules/overview) rendered. If the changes aren't reflected, try reinstalling the app.
...
Use the [Prop definition component](https://docs.expo.dev/modules/module-api/#prop) to define the prop. In the prop setter block, you can access both the view and the prop. Specify that the URL is of type `URL` &mdash; the Expo modules API will convert strings to the native `URL` type.
...
[View callbacks](https://docs.expo.dev/modules/module-api/#view-callbacks) allow developers to listen for events on components. They are typically registered through props on the component, for example: `<Image onLoad={...} />`. Use the [Events definition component](https://docs.expo.dev/modules/module-api/#events) to define an event for your WebView. Call it `onLoad`.
...
[A simple web browser UI built around our WebView](https://docs.expo.dev/static/images/modules/native-view-tutorial/web-browser.png)
