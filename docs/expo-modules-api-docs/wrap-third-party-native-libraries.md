...
_Learn how to create a simple wrapper around two separate native libraries using Expo Modules API._
...
Are you trying to use a <CODE>.aar</CODE> dependency?</>}>
...
Are you trying to use an <CODE>.xcframework</CODE> or <CODE>.framework</CODE> dependency?</>}>
...
You also need to use the [`Prop`](https://docs.expo.dev/modules/module-api/#prop) function to define the `data` prop and call the native `setChartData` function when the prop changes:
