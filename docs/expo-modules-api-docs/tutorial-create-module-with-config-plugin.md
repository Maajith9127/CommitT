...
_A tutorial on creating a native module with a config plugin using Expo Modules API._

[Config plugins](https://docs.expo.dev/config-plugins/introduction/) let you customize native Android and iOS projects generated with `npx expo prebuild` in [Continuous Native Generation (CNG)](https://docs.expo.dev/workflow/continuous-native-generation/) projects. You can use them to add properties to native config files, copy assets to native projects, or apply advanced configurations, such as adding an [app extension target](https://docs.expo.dev/build-reference/app-extensions/).

As an app developer, config plugins help you apply customizations not exposed in the default [app config](https://docs.expo.dev/workflow/configuration). As a library author, they enable you to configure native projects automatically for developers using your library.
...
To inject your custom API keys into **AndroidManifest.xml** and **Info.plist**, use helper [`mods` provided by `expo/config-plugins`](https://docs.expo.dev/config-plugins/plugins-and-mods/#what-are-mods). These make it easy to modify native files. For this example, use `withAndroidManifest` and `withInfoPlist`.
