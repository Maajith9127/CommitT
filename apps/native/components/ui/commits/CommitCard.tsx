...
export function CommitCard({
	...
	return (
		<UView
			className={`border- w-full rounded-3xl border bg-[#1A1A1A] px-4 py-5 ${className}`}
		>
			{/* ---------------------------------------------------------------- */}
			...
			<UView className="flex-row justify-between">
				...
				<UView className="w-[25%] items-start pt-1">
					<UView className="flex-row items-center rounded-full bg-[#2A2A2A] px-3 py-1">
						<MaterialCommunityIcons
							...
							color="#4FA0FF"
						/>
						<FooterText className="ml-1 font-semibold text-blue-400">
							{statusLabel}
						</FooterText>
					</UView>
				</UView>
				...
				<UView className="w-[50%] items-center">
					...
					<FooterText className="mt-1 text-center">
