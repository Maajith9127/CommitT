...
export function AuthMemberPrompt({
	...
	return (
		<UText className={`mb-4 text-center text-base text-gray-300 ${className}`}>
			{text}
		</UText>
	);
}
...
export function FooterLink({
	...
	return (
		<UButton onPress={onPress}>
			<UText className={`text-base text-blue-400 underline ${className}`}>
				{children}
			</UText>
		</UButton>
	);
...
export function AuthFooterLegal({
	...
	return (
		<UView className={`items-center ${className}`}>
			<UText className="mb-1 text-center text-gray-400 text-sm">
				{prefixText}
			</UText>
