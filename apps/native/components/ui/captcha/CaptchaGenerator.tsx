...
export function CaptchaGenerator({ difficulty, width = 280, height = 80 }: CaptchaGeneratorProps) {
  ...
  return (
    <View style={{ width, height, backgroundColor: '#111', borderRadius: 12, overflow: 'hidden', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#333' }}>
        <Svg height={height} width={width}>
            ...
            {difficulty === 'high' && (
                <>
                 <Line x1="0" y1={Math.random() * height} x2={width} y2={Math.random() * height} stroke="white" opacity={0.3} strokeWidth={2} />
                 <Line x1={Math.random() * width} y1="0" x2={Math.random() * width} y2={height} stroke="white" opacity={0.3} strokeWidth={2} />
                </>
            )}
        </Svg>
    </View>
