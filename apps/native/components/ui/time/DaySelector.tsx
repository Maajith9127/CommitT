...
export function DaySelector() {
  const { draft, addCondition, updateCondition, removeCondition } = useTaskDraftStore();

  // Find existing day condition (metric: "time", relation: "in")
  const dayCondition = draft.conditions.find((c) => c.metric === "time" && c.relation === "in");
      ...
      {days.map((day) => {
        ...
        return (
          <UPressable
            ...
            onPress={() => toggle(day)}
          >
            <UText className={`font-semibold text-sm ${isActive ? "text-black" : "text-gray-300"}`}>
              {day}
            </UText>
          </UPressable>
        );
